...
| Run full-route production SEO verification after every main deployment and daily. | Confirms Cloudflare Worker output is serving generated SEO assets and route metadata correctly. | Low | Complete: GitHub Actions runs `npm run seo:verify-production -- --all-routes` after relevant pushes, daily at 04:00 UTC and manually on demand. | Low |
