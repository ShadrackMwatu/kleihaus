...
export const buildAutomationRegistry = (generatedAt) => ({
  ...
  components: [
    ...
    { capability: 'Google Search Console performance import', status: AUTOMATION_STATUS.EXTERNAL, evidence: 'Requires verified property and approved read-only credentials', schedule: 'not active' },
    { capability: 'GA4 Data API performance import', status: AUTOMATION_STATUS.EXTERNAL, evidence: 'Client event instrumentation exists; reporting API credentials/property access are separate', schedule: 'not active' },
    { capability: 'Qualified lead and revenue attribution', status: AUTOMATION_STATUS.EXTERNAL, evidence: 'Requires private business/CRM outcomes and approved joining rules', schedule: 'not active' },
